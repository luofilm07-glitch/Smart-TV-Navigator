...
export default function Admin() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      {/* Top bar */}
      <header className="flex items-center justify-between px-6 py-3 bg-[#0f0f0f] border-b border-gray-800 sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <img src="/play-button.png" alt="" className="w-7 h-7" />
          <span className="font-bold text-lg text-white">NexStream</span>
          <span className="bg-red-600 text-white text-xs font-bold px-2 py-0.5 rounded ml-1">ADMIN</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-gray-400 text-sm">{videos.length} videos total</span>
          <a
            href="/"
            className="flex items-center gap-1.5 border border-gray-600 hover:border-blue-500 text-gray-300 hover:text-white px-3 py-1.5 rounded-lg text-sm transition-colors"
          >
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex gap-2 mb-8 border-b border-gray-800">
          {(["add", "manage"] as const).map((tab) => (
            ...
            >
              {tab === "add" ? "Upload Video" : `Manage Videos (${videos.length})`}
            </button>
          ))}
        </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full bg-[#1e1e1e] border border-gray-700 focus:border-red-500 text-white rounded-lg px-4 py-2.5 text-sm outline-none transition-colors"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full bg-[#1e1e1e] border border-gray-700 focus:border-red-500 text-white rounded-lg px-4 py-2.5 text-sm outline-none transition-colors font-mono"
                    />
                    <p className="text-gray-600 text-xs mt-1">Paste your video player embed URL (mediadelivery, YouTube embed, Vimeo embed, etc.)</p>
                  </div>

                  {/* Thumbnail URL */}
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">
                      Thumbnail Image URL <span className="text-gray-600 text-xs font-normal">(optional)</span>
                    </label>
                    <input
                      type="url"
                      ...
                      className="w-full bg-[#1e1e1e] border border-gray-700 focus:border-red-500 text-white rounded-lg px-4 py-2.5 text-sm outline-none transition-colors"
                    />
                    <p className="text-gray-600 text-xs mt-1">Leave blank to use a colored gradient thumbnail</p>
                  </div>

                  {/* Gradient picker (when no thumbnail URL) */}
                  {!form.thumbnailUrl && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {THUMB_GRADIENTS.map((g) => (
                          <button
                            type="button"
                            key={g}
                            onClick={() => set("thumbnailGradient", g)}
                            className={`w-8 h-8 rounded-lg bg-gradient-to-br ${g} transition-transform ${
                              form.thumbnailGradient === g ? "ring-2 ring-white scale-110" : "hover:scale-105"
                            }`}
                          />
                        ))}
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-[#1e1e1e] border border-gray-700 focus:border-red-500 text-white rounded-lg px-3 py-2.5 text-sm outline-none"
                      >
                        {CHANNELS.map((c) => (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-[#1e1e1e] border border-gray-700 focus:border-red-500 text-white rounded-lg px-4 py-2.5 text-sm outline-none"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full bg-[#1e1e1e] border border-gray-700 focus:border-red-500 text-white rounded-lg px-4 py-2.5 text-sm outline-none resize-none"
                    />
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={`relative w-10 h-5 rounded-full transition-colors ${form.isLive ? "bg-red-600" : "bg-gray-700"}`}
                    >
                    ...
                    {form.isLive && (
                      <input
                        type="text"
                        value={form.watchingCount}
                        onChange={(e) => set("watchingCount", e.target.value)}
                        placeholder="e.g. 1.2K watching"
                        className="flex-1 bg-[#1e1e1e] border border-gray-700 focus:border-red-500 text-white rounded-lg px-3 py-1.5 text-sm outline-none"
                      />
                    )}
                  </div>
                  ...
                  <div className="pt-2">
                    <button
                      type="submit"
                      disabled={saving || !form.title || !form.embedUrl}
                      className="w-full bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl transition-colors text-sm flex items-center justify-center gap-2"
                    >
                      {saving ? (
                        <>
                ...
                <div className="flex gap-2.5">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-white leading-snug line-clamp-2">
                      {form.title || "Video title will appear here..."}
                    </p>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-left px-4 py-3 text-gray-400 text-xs font-semibold uppercase tracking-wider hidden md:table-cell">Type</th>
                      <th className="text-left px-4 py-3 text-gray-400 text-xs font-semibold uppercase tracking-wider hidden md:table-cell">Category</th>
                      ...
                      <tr key={video.id} className={`border-b border-gray-800/60 hover:bg-white/[0.02] transition-colors ${i === videos.length - 1 ? "border-b-0" : ""}`}>
                        <td className="px-5 py-3">
                          {video.thumbnailUrl && !video.thumbnailUrl.startsWith("from-") ? (
                            ...
                            <div className={`w-12 aspect-video rounded-md bg-gradient-to-br ${video.thumbnailUrl || "from-gray-700 to-gray-900"} flex items-center justify-center`}>
                              <svg className="w-4 h-4 text-white opacity-50" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <td className="px-4 py-3 hidden lg:table-cell">
                          <div className="flex items-center gap-2">
                            <div className={`w-6 h-6 rounded-full ${CHANNEL_COLORS[video.channel] || "bg-gray-600"} flex items-center justify-center text-white text-xs font-bold`}>
                              {video.channelAvatar}
                            </div>
                        ...
                        <td className="px-4 py-3 hidden md:table-cell">
                          <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded uppercase tracking-wide
                            ${video.section === "movies" ? "bg-blue-900/50 text-blue-400"
                              : video.section === "series" ? "bg-purple-900/50 text-purple-400"
                        ...
                        <td className="px-5 py-3 text-right">
                          {deleteConfirm === video.id ? (
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <button
                                onClick={async () => { await deleteVideo(video.id!); setDeleteConfirm(null); }}
                                className="bg-red-600 hover:bg-red-700 text-white text-xs px-2.5 py-1 rounded-lg transition-colors"
                              >
                                Delete
                              </button>
                              <button
                                onClick={() => setDeleteConfirm(null)}
                                className="text-gray-400 hover:text-white text-xs px-2.5 py-1 rounded-lg border border-gray-700 transition-colors"
                              >
                                Cancel
                              </button>
                            </div>
                            ...
                            <button
                              ...
                              className="text-gray-500 hover:text-red-400 p-1.5 rounded-lg hover:bg-red-900/30 transition-colors"
                              title="Delete"
                            >
                              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
