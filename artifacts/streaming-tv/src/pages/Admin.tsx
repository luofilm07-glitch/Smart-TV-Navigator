                          ...
                          <td className="px-5 py-3 text-right">
                            {deleteConfirm === video.id ? (
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <button onClick={async () => { await deleteVideo(video.id!); setDeleteConfirm(null); }} className="bg-red-600 hover:bg-red-700 text-white text-xs px-2.5 py-1 rounded-lg transition-colors">Delete</button>
                                <button onClick={() => setDeleteConfirm(null)} className="text-gray-400 hover:text-white text-xs px-2.5 py-1 rounded-lg border border-gray-700 transition-colors">Cancel</button>
                              </div>
                            ) : (
                              <div className="flex items-center justify-end gap-1.5">
                                <button
                                  onClick={() => openEdit(video)}
                                  className="text-gray-500 hover:text-blue-400 p-1.5 rounded-lg hover:bg-blue-900/30 transition-colors"
                                  title="Edit"
                                >
                                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
